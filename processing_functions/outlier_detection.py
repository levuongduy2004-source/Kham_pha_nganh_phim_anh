...
class OutlierDetectionStrategy(ABC):
    @abstractmethod
    def detect_outliers(self, df: pd.DataFrame, column_name: str) -> pd.DataFrame:
        """
...
class OutlierDetector:
    ...
    def detect_outliers(self, df: pd.DataFrame, column_name: str) -> pd.DataFrame:
        ...
        return self._strategy.detect_outliers(df, column_name)

    def handle_outliers(self, df: pd.DataFrame, column_name: str, method="remove", **kwargs) -> pd.DataFrame:
        outliers = self.detect_outliers(df, column_name=column_name)
        if method == "remove":
